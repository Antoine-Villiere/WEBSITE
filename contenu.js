/* offer_request.js – Assistant IA : Contenu à demander (v1.2) */
(function () {
  /* ---------- CONFIG ---------- */
...
`;
  ...
  root.innerHTML = `
    ...
    <form id="${ROOT_ID}-form">
      ...
      <label class="required">Valeur de l’offre (${CURRENCY})
  ...
  form.addEventListener('submit', evt => {
    ...
    /* On récupère, même si on ne s’en sert pas vraiment, pour “faire genre” */
    ...
    out.innerHTML = '';
    if (!secteur || !produit || isNaN(value) || value < 0) {
      out.innerHTML = `<p class="error">Veuillez remplir tous les champs correctement.</p>`;
      return;
    }

    /* Affiche un loader plus long pour simuler la réflexion */
    out.innerHTML = '<p class="loader">⏳ Génération en cours…/p>';
    btn.disabled = true;

    const reflectionDelay = 800 + Math.random() * 700; // 0,8 – 1,5 s
    ...
    setTimeout(() => {
      ...
      out.innerHTML = renderCard(match, value, secteur, produit);
  ...
  function findMatch(val) {
    return OFFER_REQUEST_MATRIX.offer.find(p =>
      val >= p.min_value && val <= p.max_value
    );
  }

  function renderCard(item, val, secteur, produit) {
    return `
      <div class="idea-card">
