...
`;
  ...
  root.innerHTML = `
    <h1>Calculez votre taux d'engagement Instagram en temps réel.</h1>
    <p class="subtitle">Analysez jusqu'à 50 de vos derniers posts pour obtenir vos stats essentielles.</p>
    <form id="${ROOT_ID}-form">
      ...
      <label class="required">Nombre de posts à analyser
        <input name="posts" type="number" min="10" max="50" placeholder="Ex. : 18" required>
      </label>
  ...
  form.addEventListener('submit', async evt => {
    ...
    // Convert posts to integer, fallback to 18 if invalid
    payload.posts = parseInt(payload.posts, 10) || 18;
    ...
    try {
      ...
      out.innerHTML = renderCard(result);
    ...
    return `
      <div class="engagement-card">
        ...
        <div class="engagement-text">
          ...
          <strong>Followers :</strong> ${data.followers.toLocaleString()}<br/>
          <strong>Posts analysés :</strong> ${data.posts_analyzed}<br/>
          <strong>Likes moyens :</strong> ${data.average_likes.toFixed(1)}<br/>
          <strong>Commentaires moyens :</strong> ${data.average_comments.toFixed(2)}<br/>
          <strong>Taux d'engagement :</strong> ${data.engagement_rate_percent.toFixed(2)} %
        </div>
      </div>`;
  }
