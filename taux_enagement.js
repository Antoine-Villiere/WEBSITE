...
/* Make the first stat (Engagement) take full width */
.bento-stats .stat:first-child {
  flex: 1 1 100%;
}
/* Style the engagement value */
.bento-stats .stat:first-child .value {
  color: #005f73;
  font-weight: 700;
}
.bento-stats .stat {
...
`;
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    btn.addEventListener('click', async () => {
      ...
      try {
        ...
        showError(
          \`Oups, nous n'avons pas encore de données sur cet utilisateur, revenez très prochainement.\`,
          container,
          username
        );
      }
  ...
  function renderCard(el, d) {
    const escapeHtml = html =>
      (html || '').toString().replace(/[&<>'"]/g, c =>
        ({ '&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;' })[c]);

    // Stats array with Engagement first
    const stats = [
    ...
    el.innerHTML = \`
      <div class="bento-card">
        <div class="bento-header">
          <img src="\${escapeHtml(proxyUrl(d.profile_pic_url))}"
               alt="@\${escapeHtml(d.username)}"
               loading="lazy"
        ...
        <div class="bento-warning">
          <strong>Attention&nbsp;:</strong> @\${escapeHtml(d.username)} peut volontairement cacher ses statistiques, ce qui peut impacter les données ci-dessous.
        </div>
        ...
        <button class="bento-collab-btn"
                onclick="window.open('https://www.spottedge.app','_blank')">
