...
/* Nouveau style pour le bouton de collaboration */
.bento-collab-btn {
...
.bento-collab-btn:hover {
  ...
  // ---- Affiche le message d’erreur générique + bouton de collaboration ----
  function displayFallback(user) {
    out.innerHTML = `
      <p class="error">
        Oups, nous n'avons pas encore de données sur cet utilisateur, revenez très prochainement.
      </p>
      <button id="collab-btn" class="bento-collab-btn">
        Commencer à collaborer avec @${user}
      </button>
    `;
    const collab = document.getElementById('collab-btn');
    collab.addEventListener('click', () => {
      window.open('https://www.spottedge.app', '_blank');
    });
  }
