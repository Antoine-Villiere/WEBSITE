  ...
  function renderCard(d) {
    ...
    card.innerHTML = `
      <div class="bento-header">
        <img src="${esc(d.profile_pic_url)}" alt="@${esc(d.username)}" loading="lazy" onerror="this.src='https://via.placeholder.com/72?text=User';">
      ...
      <div class="bento-stats">
        ${[
          ...
          `<div class="stat"><span class="value">${esc(m.value)}</span><span class="label">${m.label}</span></div>`
        ).join('')}
      </div>
      <div class="bento-feed">
        ${(d.recent_posts || []).slice(0, 6).map(url =>
          `<img src="${esc(url)}" loading="lazy" onerror="this.src='https://via.placeholder.com/80';" alt="">`
        ).join('')}
      </div>
    `;
    return card;
